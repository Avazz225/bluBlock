...
class AccountOverview extends ChangeNotifier{
  ...
  _initValues() async {
    ...
    List accountNames = await _db.readDB('account', ['account_id', 'account_name', 'blocked', 'category_id', 'platform_id'], 'category_id  <= ?', [maxBlockLevel], 'category_id ASC, account_name ASC', 1000000);
  ...
  getAccountList(){
    ...
    for (final acc in _accounts){
      result.add({"accountName": acc.accountName, "blocked":acc.blocked, "platform":_platforms[acc.platformId], "category":_categories[acc.categoryId]});
    }
    return result;
  }
